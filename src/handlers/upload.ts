...
import { EXT_BY_TYPE, fullKey, makeId, randSuffix, thumbKey } from "../ids.js";

const MAX_FULL_BYTES = 25 * 1024 * 1024;

export async function handleUpload(request: Request, ctx: AppContext): Promise<Response> {
  ...
  await ctx.storage.put(fullKey(id, ext), full.stream(), {
  ...
  if (hasThumb) {
    ...
    await ctx.storage.put(thumbKey(id), thumb.stream(), {
