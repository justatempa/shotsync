...
import { epochMsFromId, idFromFullKey } from "../ids.js";
...
export async function handleList(request: Request, ctx: AppContext): Promise<Response> {
  ...
  const res = await ctx.storage.list({ prefix: "full/", limit, cursor });
  ...
  const items = res.objects.map((o) => {
    ...
    return {
      ...
      hasThumb: o.metadata?.hasThumb === "true",
      source: o.metadata?.source || "unknown",
    };
  });
  ...
  // local disk reads and cost milliseconds, whereas the fetches they replace
  // crossed the network from wherever the visitor happens to be.
  const textItems = items.filter((i) => i.contentType.startsWith("text/"));
  await Promise.all(
    textItems.slice(0, MAX_INLINE_SNIPPETS).map(async (item) => {
      try {
        const obj = await ctx.storage.get(item.key);
        if (!obj) return;
  ...
  // `key` is an internal storage path; it exists only to fetch the snippet above
