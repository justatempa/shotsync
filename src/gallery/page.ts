import { maskToken } from "./settings.js";
...
<meta name="description" content="shotsync 跨设备图片与文字池。在自己服务器上上传截图、照片和文字，从另一台设备取回。">
...
const isLatin1 = (s) => [...s].every((c) => c.charCodeAt(0) <= 0xFF);

$("#tokenSave").onclick = async () => {
  token = $("#tokenInput").value.trim();
  if (!token) return;
  // fetch headers only allow ISO-8859-1 characters; AUTH_TOKEN is ASCII, so a
  // Chinese-character paste would crash the fetch call with a cryptic error.
  if (!isLatin1(token)) { showGate("令牌不能包含中文等非 Latin-1 字符，请核对服务器 AUTH_TOKEN"); return; }
...
// Read-only demo pool: point visitors at self-hosting without enabling writes.
async function enterDemo() {
  ...
  link.target = "_blank";
  link.rel = "noreferrer";
  link.textContent = DEMO_EN ? "Self-host your own →" : "部署自己的实例 →";
...
(async function boot() {
  ...
  if (token && isLatin1(token) && await apiOk()) { showApp(); setupUpload(); await initFeed(); }
  else { showGate(); }
})();
</script>
</body>
